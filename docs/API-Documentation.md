...
7. [Typical frontend flows](#typical-frontend-flows)
8. [Changes in this release (breaking for clients)](#changes-in-this-release-breaking-for-clients)
...
These flows only chain the endpoints; the request and response details are in each linked section.

### First launch and placement (Child app)

1. **Create a session.** Call [`POST /api/auth/guest`](#register-guest) once, or [register](#register-with-email) / [log in](#login-with-email) / [Google](#google-sign-in--sign-up). Store `userId`, `accessToken` and `refreshToken` securely. After Google sign-in with `age: null`, ask for the age and call [`PATCH /api/auth/age`](#set-age).
2. **Check placement.** Call [`GET /api/placement`](#placement-status) and branch on `status`:
   - `Required`: show the test.
   - `Optional`: offer the test.
   - `InProgress`: resume it (step 3).
   - `Completed`: go to `result.levelId`.
   - `Unavailable`: skip placement.
3. **Start or resume.** Call [`POST /api/placement/start?language=ar`](#start-or-resume-placement-test). It returns the same `attemptId` while an open attempt has not expired. After an app restart, redraw it with [`GET /api/quiz-attempts/{attemptId}`](#get-attempt-resume).
4. **Submit.** Call [`POST /api/quiz-attempts/{attemptId}/submit`](#submit-attempt), with **every** question in `mistakes` (placement has no essays). Read `placement.levelId` from the result. If the response is lost, call [`GET …/result`](#get-saved-result-recovery-and-essay-polling).
5. **Show the level map.** Call [`GET /api/content/levels`](#list-levels) and highlight the placed level.
6. **Upgrade the guest later.** Call [`POST /api/auth/register`](#register-with-email) with `existingGuestUserId`, replace the stored tokens, then [`POST /api/auth/logout`](#logout) with the old guest refresh token.

### Lesson → quiz → submit → poll → retry with hints (Child app)

1. **Pick a lesson.** Call [`GET /api/content/levels/{levelId}/lessons`](#list-lessons-of-a-level) and hide lessons with `isPublished: false`. Open one with [`GET /api/content/lessons/{id}`](#get-lesson-detail).
2. **Preview the quiz.** Call [`GET /api/quizzes/for-lesson/{lessonId}`](#lesson-quiz-preview). A 404 means the lesson has no quiz: hide the quiz entry point.
3. **Start.** Call [`POST /api/quiz-attempts?quizId={quizId}`](#start-attempt-first-attempt-or-retry) and render the questions from **this** response. Keep `attemptId`.
4. **Hints (optional).** While answering, call [`POST /api/quiz-attempts/{attemptId}/questions/{questionId}/hint`](#hint-button). Level 1, then level 2; a further press returns 409. Disable the button while a request is in flight.
5. **Submit.** Call [`POST /api/quiz-attempts/{attemptId}/submit`](#submit-attempt) with every MultipleChoice/TrueFalse answer in `mistakes` and every essay in `essayAnswers`. Set a client timeout of 30–45 s, because the call waits up to about 15 s for the AI.
   - Show `scorePercentage` (final at once) and `earnedPoints` / `totalPoints`.
   - On a timeout or lost response, recover with [`GET …/result`](#get-saved-result-recovery-and-essay-polling): 200 shows the result, 409 means resubmit, 410 means start again.
6. **Poll essays.** While `pendingEssayQuestions > 0`, call [`GET /api/quiz-attempts/{attemptId}/result`](#get-saved-result-recovery-and-essay-polling) every 1–2 minutes while the screen is visible, and on pull-to-refresh. Stop at `0`. Each essay ends `Graded` (points and feedback) or `NotGraded` (no points, final).
7. **Retry.** If `wrongAnswers > 0`, show `retryQuestions` with their `currentHint`. Then call [`POST /api/quiz-attempts?quizId={quizId}&previousAttemptId={attemptId}`](#start-attempt-first-attempt-or-retry).
   - The new attempt holds only the still-active wrong questions, with the latest hints. Keep those hints from this response.
   - Submit it like any attempt. Each attempt can be retried once; a retry can be retried again while it still has wrong answers.
8. **Refresh progress.** Call [`GET /api/user-topic-stats?language=ar`](#my-topic-statistics) after a submit to redraw the progress map (XP, stars, mastery, messages). Call it again when step 6 finds a newly `Graded` essay, because its points add XP. For a topic detail screen, use [`GET /api/user-topic-stats/{topicId}`](#one-topics-progress).

### Admin: set up categories and topics

1. **Categories.** List them with [`GET /api/assessment/categories`](#list-assessment-categories). Create missing ones with [`POST /api/assessment/categories`](#create-assessment-category); `sortOrder` sets their order on the child's progress map.
2. **Topics.** Create each topic in its category with [`POST /api/assessment/topics`](#create-assessment-topic), with a `learningLevel` and `ar`/`en` translations so the child sees names in both languages.
3. **Edit.** Always send whole objects to `PUT`, and `translations: null` to keep the stored translations. Retire a topic or category with its `PATCH …/active?isActive=false` endpoint instead of deleting it.

### Admin: create a question with an image

1. **Pick a quiz.** Use [`GET /api/quizzes?quizType=LessonQuiz&lessonId=5`](#list-quizzes-paged), or [`POST /api/quizzes`](#create-quiz) to create one. A `Placement` quiz cannot own questions.
2. **Upload the image.** Call [`POST /api/content/media/images`](#upload-image) (multipart field `file`, jpg/jpeg/png/webp, 5 MB or less) and keep `data.url`.
3. **Create the question.** Call [`POST /api/questions`](#create-question) with:
   - `imageUrl` = `data.url`
   - a **mandatory** `imageDescription` (non-blank, 1000 characters or less, describing what the image shows, because the AI never sees the image)
   - `questionType`, `difficulty`, a free `displayOrder`, and `points` (omit it for 1)
   - `topicId` picked from [`GET /api/assessment/topics?isActive=true`](#list-assessment-topics), or `null` for a question with no topic

   The question is created **inactive**.
4. **Add options.** For MultipleChoice/TrueFalse, call [`POST /api/question-options`](#create-option) once per option: exactly one `isCorrect: true`, and TrueFalse exactly 2. An image option also needs its own `imageDescription`. Skip this step for Essay.
5. **Activate.** Call [`PATCH /api/questions/{questionId}/active?isActive=true`](#activate--deactivate-question). A 400 lists what is missing: option count, the one correct answer, or undescribed images.
6. **Refresh the editor.** Call [`GET /api/questions?quizId={quizId}`](#list-questions-of-a-quiz). For later edits, always send the whole object to [`PUT /api/questions/{questionId}`](#update-question), because omitted fields reset.

---

## Changes in this release (breaking for clients)

Compared with the previous commit (`HEAD`), these changes affect what clients send or receive:

1. **`essayResults[].gradedBy` is removed** from `EssayResultDto`. It used to be `"Ai"` or `"Human"`. Essays are now graded by the AI only; there is no teacher or reviewer role, no model answer and no rubric.
2. **Essay status has a new final value.** `essayResults[].status` is `Pending | Graded | NotGraded`; it used to be `Pending | Graded`, where Pending could mean "waiting for a person".
   - `NotGraded` is final. It means the AI declined (answered `Skipped`) or produced no usable grade after the allowed attempts. `awardedPoints` and `feedback` are `null`, and nobody else will grade it.
   - Treat any unknown status as not final.
3. **New result fields `totalPoints`, `earnedPoints` and `pendingPoints`** on `QuizAttemptResultDto` (submit and `GET …/result`).
   - `totalPoints` covers every question, essays included.
   - `earnedPoints` is the points of correct MultipleChoice/TrueFalse answers plus the awarded points of Graded essays. It **grows** while essays are Pending.
   - `pendingPoints` is the `maxPoints` of Pending essays.
   - `correctAnswers` and `wrongAnswers` stay **counts**.
4. **`scorePercentage` is weighted by points.**
   - Formula: points of correct MultipleChoice/TrueFalse ÷ points of all MultipleChoice/TrueFalse × 100, 2 decimal places.
   - Points are frozen per attempt at start (`QuizAttemptQuestions.Points`), and a question's points default to 1.
   - Essays never enter this number, so it is final at submit. It equals the plain correct ÷ total ratio only when every auto-graded question is worth 1 point.
5. **Placement is per-level by points.** `placement.levels[]` (and `GET /api/placement` → `result.levels[]`) gains `totalPoints` and `earnedPoints`.
   - A level is mastered when earned ÷ total points of its placement questions ≥ `passPercentage`.
   - The placement `scorePercentage` is points-weighted too.
   - `UserTopicStats` still count questions, not points.
6. **Another user's attempt returns 404, not 403.** This applies to get, result, submit, retry (`previousAttemptId`) and hint. The message is exactly the one used for a nonexistent id, for example `المحاولة رقم 42 غير موجودة`. Previously these returned 403 with messages such as `لا يمكنك الوصول إلى محاولة مستخدم آخر` ("you cannot access another user's attempt"). A 403 now only means a role restriction.
7. **`imageDescription` is mandatory whenever an image is set** on `POST`/`PUT /api/questions` and `POST`/`PUT /api/question-options` (non-blank, 1000 characters or less). It is cleared when there is no image.
   - Activation refuses questions whose question or option images lack descriptions.
   - Migration `db/migrations/001_points_ai_essays_image_descriptions.sql` deactivates active questions with undescribed images.
   - `imageDescription` is admin-only and never appears in child responses.
8. **Internal errors are now a generic 500.** `InvalidOperationException` used to map to **400 with its raw message**. It is no longer mapped, so framework and EF Core faults return **500** with `حدث خطأ داخلي في الخادم`.
   - Business rules now throw `BusinessRuleException` (`Shared/Common/Exceptions`), which is still 400 with its message.
   - The image upload (`MediaController.Upload`) likewise returns 400 only for its own rules (empty, too large, wrong extension). A server-side storage failure is now a 500.
   - Clients should not display 500 messages as validation feedback.
9. **Topic statistics are now a progress map.** `GET /api/user-topic-stats` returns one `MyProgressResponseDto` **object** instead of an array of `UserTopicStatResponseDto` rows (that DTO is deleted).
   - The row fields `id`, `userId`, `topicId`/`difficulty` rows, `questionsAnsweredCount`, `correctCount`, `wrongCount`, `hintsUsedCount`, `lastQuizAttemptId` and `updatedAt` are gone. The same counts now appear per topic and per difficulty as `questionsAnswered`, `correctAnswers`, `wrongAnswers` and `hintsUsed`.
   - New: `totalXp` and per-topic `xp`, `mastery` (`NotStarted | Learning | Practicing | Mastered`), `stars` (0–3), `accuracyPercentage`, topic and category names, and encouraging `message` texts.
   - Every active topic is listed, including topics the child has not started. A new child gets a full map of `NotStarted` topics, not `[]`.
   - The endpoint is now **localized** (`?language=` → `Accept-Language` → `ar`).
10. **`GET /api/user-topic-stats/{topicId}/{difficulty}` is removed.** Use `GET /api/user-topic-stats/{topicId}`, which returns one topic with its `difficulties[]`. A call to the old route matches no route (404, empty body).
    - The old ProblemDetails 404 is gone. The new route answers 200 `NotStarted` for a topic not practised yet, and a 404 envelope `الموضوع رقم {id} غير موجود` only when the topic does not exist.
11. **A question's topic is optional.** `topicId` is `int | null` in `AdminQuestionResponseDto`, `CreateQuestionDto` and `UpdateQuestionDto`.
    - `POST /api/questions` without `topicId` creates a question with no topic; it used to be a 404.
    - `PUT /api/questions/{questionId}` with `topicId` null or omitted **clears** the topic.
    - Admin clients must handle `topicId: null` in responses. AI requests carry `topic: null` for such a question.
12. **`hintsRemaining` is added** to the Hint button response (`HintResponseDto`): levels left after this press, unchanged by `Partial` and `Unavailable`. Two concurrent presses on the same question now get one 200 and one 409 in **any** language; before, two presses in different languages could both use a level.
13. **New admin endpoints for categories and topics:** `GET`/`POST /api/assessment/categories`, `GET`/`PUT /api/assessment/categories/{categoryId}`, `PATCH /api/assessment/categories/{categoryId}/active`, and the same five for `/api/assessment/topics`. There are no delete endpoints.
14. **Database migration `db/migrations/002_optional_topics_hint_level_uniqueness.sql` is required** for databases built from an earlier `000_AssessmentSchema.sql` (run it after 001). It makes `Questions.TopicId` and `QuizAttemptQuestions.TopicId` nullable and adds `UQ_QuestionHints_AttemptId_QuestionId_AttemptNumber`, renumbering levels that races had already duplicated. Deploy it together with this code: the old code cannot read a question without a topic.
15. **New `GET /api/quiz-attempts/latest`** (not breaking). Returns the result of the caller's most recently completed attempt, the same body as `GET /api/quiz-attempts/{attemptId}/result`, with no parameters; 404 when the caller has not completed any attempt.